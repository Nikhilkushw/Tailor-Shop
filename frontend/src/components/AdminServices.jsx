...
const AdminServices = () => {
  ...
  const handleAddService = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${baseURL}/api/services`, newService);
      setServices([...services, res.data]);
      setNewService({ heading: "", description: "" });
    } catch (err) {
  ...
  const handleUpdateService = async (e) => {
    ...
    try {
      const res = await axios.put(
        `${baseURL}/api/services/${editingService._id}`,
        editingService
      );
      setServices(
  ...
  return (
    <div className="container mx-auto px-6 py-10">
      ...
      {/* Add New Service */}
      <form onSubmit={handleAddService} className="bg-white shadow-md rounded-lg p-6 mb-8">
      ...
      <div className="grid gap-6">
        {services.map((service) => (
          <div key={service._id} className="p-6 border rounded-lg shadow bg-gray-50">
            {editingService?._id === service._id ? (
              ...
              <>
                ...
                <div className="mt-4 flex gap-3">
                  <button onClick={() => setEditingService(service)} className="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600">
                    Edit
                  </button>
                  <button onClick={() => handleDeleteService(service._id)} className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">
                    Delete
                  </button>
                </div>
              </>
