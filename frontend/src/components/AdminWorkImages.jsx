...
export default function AdminWorkImages() {
  ...
  const fetchWorks = async () => {
    ...
    try {
      ...
      // If backend wraps works inside an object, use:
      setWorks(Array.isArray(res.data) ? res.data : res.data.works || []);
    } catch (err) {
  ...
  const handleUpdateWork = async (e) => {
    e.preventDefault();
    const cloudUrl =
      editingWork.sampleImage instanceof File
        ? await uploadToCloudinary(editingWork.sampleImage)
        : editingWork.sampleImage;
  ...
  const handleAddItem = async (workId) => {
    const cloudUrl = newItem.image
      ...
      : null;
    try {
      await axios.post(`${API}/${workId}/items`, {
        title: newItem.title,
  ...
  const handleUpdateItem = async (workId, e) => {
    ...
    const cloudUrl =
      editingItem.image instanceof File
        ? await uploadToCloudinary(editingItem.image)
        : editingItem.image;
                  ...
                  {editingItem?._id === item._id ? (
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => handleItemChange(e, true)}
                        className="border p-2 rounded"
                      />
                      <textarea
                        name="description"
